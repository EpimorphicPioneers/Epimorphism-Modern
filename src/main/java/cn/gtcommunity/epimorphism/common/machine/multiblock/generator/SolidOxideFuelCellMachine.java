...
public class SolidOxideFuelCellMachine extends TieredWorkableElectricMultiblockMachine {
    ...
    private static final ManagedFieldHolder MANAGED_FIELD_HOLDER = new ManagedFieldHolder(SolidOxideFuelCellMachine.class, TieredWorkableElectricMultiblockMachine.MANAGED_FIELD_HOLDER);

    private static final FluidStack MK1_STEAM = GTMaterials.Steam.getFluid(20 * FluidHelper.getBucket());

    private static final FluidStack MK2_STEAM = GTMaterials.Steam.getFluid(96 * FluidHelper.getBucket());

    private static final FluidStack MK1_OXYGEN = GTMaterials.Oxygen.getFluid(FluidHelper.getBucket() / 10);

    private static final FluidStack MK2_OXYGEN = GTMaterials.Oxygen.getFluid(2 * FluidHelper.getBucket());
    ...
    protected @Nullable GTRecipe getRealRecipe(GTRecipe recipe) {
        ...
        GTRecipe copied = recipe.copy(ContentModifier.multiplier((double) calorificValue / total), false);
        copied.duration = 20;
    ...
    public static ResourceLocation getTexture(int tier) {
        return tier == HV ? GTCEu.id("block/casings/solid/machine_casing_clean_stainless_steel") : GTCEu.id("block/casings/solid/machine_casing_robust_tungstensteel");
    }
    ...
    public static Block getElectrolyteUnitState(int tier) {
        return tier == HV ? Y_ZR_CERAMIC_ELECTROLYTE_UNIT.get() : GD_CE_CERAMIC_ELECTROLYTE_UNIT.get();
    }
}
