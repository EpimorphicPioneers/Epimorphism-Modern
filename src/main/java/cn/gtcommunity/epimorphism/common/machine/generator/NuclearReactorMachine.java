...
import cn.gtcommunity.epimorphism.api.gui.widget.DisabledSlotWidget;
...
public class NuclearReactorMachine extends MetaMachine implements IFancyUIMachine, IMachineLife, IWorkable, IMachineModifyDrops {
    protected static final ManagedFieldHolder MANAGED_FIELD_HOLDER = new ManagedFieldHolder(NuclearReactorMachine.class, MetaMachine.MANAGED_FIELD_HOLDER);
    @Getter
    @Persisted
    ...
    protected NotifiableItemStackHandler createInventoryItemHandler() {
        return new NotifiableItemStackHandler(this, 54, IO.BOTH, IO.BOTH, ReactorItemStackTransfer::new);
    }

    public boolean shouldOpenUI(Player player, InteractionHand hand, BlockHitResult hit) {
        return !player.getMainHandItem().is(EPMachines.REACTOR_CHAMBER.getItem()) && IFancyUIMachine.super.shouldOpenUI(player, hand, hit);
    }
    ...
    public Widget createUIWidget() {
        ...
        for(int y = 0; y < colSize; ++y) {
            for(int x = 0; x < rowSize; ++x) {
                int slot = index;
                slotContainer.addWidget((new DisabledSlotWidget(this.getInventory().storage, index++, 4 + x * 18, 4 + y * 18, () -> handler.isLock(slot), (b) -> {
        ...
        barContainer.addWidget(new ImageWidget(0, 0, 18 * rowSize + 8, 10, EPGuiTextures.BAR_CONTAINER) {
            @Override
        ...
        var reactorHeat = new ProgressWidget(() -> (double) heat / maxHeat, 3, 3, 164, 4, EPGuiTextures.PROGRESS_BAR_REACTOR_HEAT);
    ...
    public static class ReactorContext {
        ...
        @Nullable
        public IReactorComponent getComponent(int row, int column) {
    ...
    private class ReactorItemStackTransfer extends LockableItemStackTransfer {
        ...
        public ItemStack getStackIn(int row, int column) {
            return row >= 0 && row < 6 && column >= 0 && column < 9 ? this.getStackInSlot(row * 9 + column) : ItemStack.EMPTY;
        }
