...
public class BallHatchPartMachine extends TieredPartMachine implements IMachineModifyDrops {
    ...
    protected static final ManagedFieldHolder MANAGED_FIELD_HOLDER = new ManagedFieldHolder(BallHatchPartMachine.class, TieredPartMachine.MANAGED_FIELD_HOLDER);
    ...
    public Widget createUIWidget() {
        ...
        for (int y = 0; y < colSize; y++) {
            for (int x = 0; x < rowSize; x++) {
                int slot = index;
                container.addWidget(new LockableSlotWidget(handler, index++, 4 + x * 18, 4 + y * 18, () -> handler.isLock(slot), b -> handler.setLock(slot, b))
