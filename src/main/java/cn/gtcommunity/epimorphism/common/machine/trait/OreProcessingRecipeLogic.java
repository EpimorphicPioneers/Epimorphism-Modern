...
public class OreProcessingRecipeLogic extends RecipeLogic {
    ...
    private static final FluidStack DISTILLED_WATER = GTMaterials.DistilledWater.getFluid(Long.MAX_VALUE);
    private static final FluidStack LUBRICANT = GTMaterials.Lubricant.getFluid(Long.MAX_VALUE);
    private static final TagKey<Item> STONE_DUST = ChemicalHelper.getTag(TagPrefix.dust, GTMaterials.Stone);
    private static final TagKey<Item> END_STONE_DUST = ChemicalHelper.getTag(TagPrefix.dust, GTMaterials.Endstone);
    private static final TagKey<Item> NETHER_RACK_DUST = ChemicalHelper.getTag(TagPrefix.dust, GTMaterials.Netherrack);
    ...
    @Nullable
    private GTRecipe getProcessingRecipe() {
        ...
        for (ItemStack stack : inputItems) {
            if (charged <= 0) break;
            if (checkTags(stack.getItem(), ore, rawOre, crushed, crushedPurified, crushedRefined, dustImpure, dustPure)) {
    ...
    private long getFluidAmount(FluidStack fluid) {
        ...
        return GTTransferUtils.drainFluidAccountNotifiableList(getInputTank(), fluid.copy(Long.MAX_VALUE), true).getAmount();
    }
