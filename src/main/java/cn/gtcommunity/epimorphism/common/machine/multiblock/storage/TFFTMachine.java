...
public class TFFTMachine extends WorkableMultiblockMachine implements IFancyUIMachine, IDisplayUIMachine, ITankProvider {
...
//        this.fluidTank = new TFFTFluidTank(this, BigInteger.ZERO, 1);
    ...
    public void onStructureFormed() {
        ...
        if (container instanceof StorageFieldBlockContainer blockContainer) {
            int tanks = BASE_DISTINCT_FLUIDS + (blockContainer.getBlockNumber() / 9) * DISTINCT_FLUIDS_PER_LAYER;
...
//                    traitSubscriptions.add(handler.addChangedListener(tickSubscription::updateSubscription));
    ...
    private boolean onTankDirty(TFFTFluidTank tank) {
        if (tank != null) {
            ...
            return tank.getSyncStorage().hasDirtySyncFields() || tank.getSyncStorage().hasDirtyPersistedFields();
        }
        return false;
    }
    ...
    public static class TFFTFluidTank extends MachineTrait implements ITankTrait {
        ...
        public TFFTFluidTank rebuild(@NotNull BigInteger capacity, int tanks) {
            ...
            var storages = Arrays.stream(this.storages).filter(TankStorage::hasFluid).toArray(TankStorage[]::new);
