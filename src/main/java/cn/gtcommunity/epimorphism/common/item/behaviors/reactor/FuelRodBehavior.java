...
public class FuelRodBehavior implements IReactorComponent {
    ...
    public FuelRodBehavior(final int maxDurability, final int energyMult, final float heatMult, final int rodCount, final boolean moxStyle) {
    ...
    protected int handleHeat(NuclearReactorMachine.ReactorContext context, final int heat) {
        ...
        if (heatableNeighbors.isEmpty()) {
            ...
            for (var heatableNeighbor : heatableNeighbors.entrySet()) {
                int accepted = heatableNeighbor.getValue().adjustCurrentHeat(heatableNeighbor.getKey(), tHeat + remainderHeat);
