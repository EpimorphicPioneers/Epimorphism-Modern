...
import cn.gtcommunity.epimorphism.core.mixins.accessors.GTRecipeTypeAccessor;
...
public class GTRecipeManager {
    ...
    public static void onGTPostInitialization() {
        Map<GTRecipeType, List<BiConsumer<GTRecipeBuilder, Consumer<FinishedRecipe>>>> map = new HashMap<>();
        ModLoader.get().postEvent(new GTRecipeEvent.RegisterHandler(map));
        ImmutableMap<GTRecipeType, List<BiConsumer<GTRecipeBuilder, Consumer<FinishedRecipe>>>> handlerList = ImmutableMap.copyOf(map);
