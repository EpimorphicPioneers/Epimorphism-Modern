...
public class BouleRecipeHandler {
    ...
    private static void processCrystallization(TagPrefix gem, @Nonnull Material material, GemProperty property, Consumer<FinishedRecipe> provider) {
        ...
        if (!material.hasFlag(CRYSTALLIZABLE) || material.hasFlag(DISABLE_CRYSTALLIZATION))
            return;

        //  If material has too may components, then do not generate recipe.
        if (material.getMaterialComponents().size() > CRYSTALLIZATION_RECIPES.getMaxInputs(ItemRecipeCapability.CAP) - 1 + CRYSTALLIZATION_RECIPES.getMaxInputs(FluidRecipeCapability.CAP)) {
            return;
        }
        ...
        GTRecipeBuilder builder = CRYSTALLIZATION_RECIPES.recipeBuilder("%s_crystallization".formatted(material.getName().toLowerCase(Locale.ROOT)))
        ...
        //  Seed Crystal -> Boule
        builder.inputItems(seedCrystal, material, componentAmount).outputItems(boule, material, componentAmount);
        ...
        CUTTER_RECIPES.recipeBuilder("%s_boule_to_gem".formatted(material.getName().toLowerCase(Locale.ROOT)))
        ...
        AUTOCLAVE_RECIPES.recipeBuilder("%s_seed_crystal".formatted(material.getName().toLowerCase(Locale.ROOT)))
