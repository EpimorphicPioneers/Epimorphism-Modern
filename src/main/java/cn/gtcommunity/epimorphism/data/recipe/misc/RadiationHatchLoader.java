...
public class RadiationHatchLoader {
    ...
    private static void registerRadioactiveMaterial(Material material, int duration, int radioactivity, Consumer<FinishedRecipe> provider) {
        if (material.hasFlag(MaterialFlags.GENERATE_ROD)) {
            EPRecipeTypes.RADIATION_HATCH_LIST.recipeBuilder("radioactive_material_rod.%s".formatted(material.getName().toLowerCase(Locale.ROOT)))
        ...
        if (material.hasFlag(MaterialFlags.GENERATE_LONG_ROD)) {
            EPRecipeTypes.RADIATION_HATCH_LIST.recipeBuilder("radioactive_material_long_rod.%s".formatted(material.getName().toLowerCase(Locale.ROOT)))
