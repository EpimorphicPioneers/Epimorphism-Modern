...
public class PlanetDisplayRenderer extends TextureOverrideRenderer {
    ...
    public PlanetDisplayRenderer(String planetName, int tier) {
        super(new ResourceLocation("block/cube"), Map.of("up", Epimorphism.id("block/planet/%s/top".formatted(planetName)),
                "down", Epimorphism.id("block/planet/%s/bottom".formatted(planetName)),
                "north", Epimorphism.id("block/planet/%s/back".formatted(planetName)),
                "south", Epimorphism.id("block/planet/%s/front".formatted(planetName)),
                "east", Epimorphism.id("block/planet/%s/right".formatted(planetName)),
                "west", Epimorphism.id("block/planet/%s/left".formatted(planetName)),
                "particle", Epimorphism.id("block/planet/%s/front".formatted(planetName))));
        this.tier = tier;
    }
    ...
    public void renderItem(ItemStack stack, ItemDisplayContext transformType, boolean leftHand, PoseStack poseStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay, BakedModel model) {
        ...
        if (transformType == ItemDisplayContext.GUI) {
            ...
            fontRender.drawInBatch(subscript, 0.0F, (16.0F / smallTextScale) - 9.0F + 1.0F, 16777215, true, poseStack.last().pose(), buffer, DisplayMode.NORMAL, 0, LightTexture.FULL_BRIGHT);
            poseStack.popPose();
        }
    }
