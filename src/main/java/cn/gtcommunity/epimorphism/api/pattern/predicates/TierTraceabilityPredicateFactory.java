        ...
        return switch (type) {
            case TIER -> new TraceabilityPredicate(new EnhancePredicate(
                    getTierPredicate(tName,
                            EPUtil.getOrDefault(map, Object2ObjectOpenHashMap<ITierType, Supplier<Block>>::new),
                            EPUtil.getOrDefault(container, () -> IValueContainer::noop),
                            EPUtil.getOrDefault(errorKey, () -> Component.translatable("structure.multiblock.pattern.error.casing"))
                    ),
                    getCandidates(tName,
                            EPUtil.getOrDefault(candidatesMap != null ? candidatesMap : map, Object2ObjectOpenHashMap<ITierType, Supplier<Block>>::new),
                            EPUtil.getOrDefault(comparator, () -> Comparator.comparingInt(ITierType::tier)),
                            EPUtil.getOrDefault(predicate, () -> BlockState -> true)
                    )).setBlockTier(true));
