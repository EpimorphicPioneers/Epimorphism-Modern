package cn.gtcommunity.epimorphism.api.event;
...
public abstract class GTRecipeEvent extends Event {
    ...
    public static class RemoveRecipe extends GTRecipeEvent implements IModBusEvent {
        ...
        public void removeRecipesByInputs(GTRecipeType type, ItemStack[] itemInputs, FluidStack[] fluidInputs) {
        ...
        public void removeRecipesByOutputs(GTRecipeType type, ItemStack[] itemOutputs, FluidStack[] fluidOutputs) {
            this.map.computeIfAbsent(type, t -> new ArrayList<>()).add(b -> {
        ...
        private boolean checkItemsMatch(ItemStack[] itemStacks, List<Ingredient> list) {
            return Arrays.stream(itemStacks).allMatch(stack -> list.stream().anyMatch(item -> item.test(stack)));
        }

        private boolean checkFluidsMatch(FluidStack[] fluidStacks, List<FluidIngredient> list) {
            return Arrays.stream(fluidStacks).allMatch(stack -> list.stream().anyMatch(fluid -> fluid.test(stack)));
        }
    }

    public static class RegisterHandler extends GTRecipeEvent implements IModBusEvent {
        private final Map<GTRecipeType, List<BiConsumer<GTRecipeBuilder, Consumer<FinishedRecipe>>>> map;

        @ApiStatus.Internal
        public RegisterHandler(Map<GTRecipeType, List<BiConsumer<GTRecipeBuilder, Consumer<FinishedRecipe>>>> map) {
            this.map = map;
        }

        public void register(GTRecipeType type, BiConsumer<GTRecipeBuilder, Consumer<FinishedRecipe>> handler) {
