...
public class ComponentAsslineRecipeHandler {
    ...
    private static final Object2ObjectOpenHashMap<Item, RecipeRecord> recipeRecords =
            new Object2ObjectOpenHashMap<>();
    private static final Object2ObjectOpenHashMap<Item, List<ItemStack>> inputComponents =
            new Object2ObjectOpenHashMap<>();
    ...
    public static void finish(Consumer<FinishedRecipe> provider) {
        recipeRecords.forEach((outputItem, record) -> record
                .inputComponentItems()
                .forEach((recipeRecord, count) -> copyToRecord(record, recipeRecord, count, 11)));
        ...
        recipeRecords.forEach((outputItem, record) -> {
            ...
            var builder = COMPONENT_ASSEMBLY_LINE_RECIPES
                    .recipeBuilder("component_%s".formatted(outputItem.toString()))
                ...
                for (Ingredient.Value value : ((IngredientAccessor) sized.getInner()).getValues()) {
                    if (value instanceof Ingredient.TagValue tagValue) {
                        ...
                        if (input.chance == 0) {
                            value
                                    .getItems()
                                    .forEach(stack -> addCableToList(inputItems, stack.copyWithCount(count)));
                        } else {
                            value
                                    .getItems()
                                    .forEach(stack -> addCableToList(inputItems, stack.copyWithCount(count * 48)));
                        }
                    }
                }
            } else {
        ...
        while (componentsIterator.hasNext()) {
            ...
            while (rlistIterator.hasNext()) {
                ...
                if (stack.getItem() == outputItem) {
                    addToMap(
                            recipeRecords.get(entry.getKey()).inputComponentItems(), record, stack.getCount());
    ...
    private static boolean handlerTag(
            TagKey<Item> tag, int count, AbstractObject2IntMap<UnificationEntry> map) {
    ...
    private static void addFluidToList(List<FluidIngredient> list, FluidStack fluidStack) {
        ...
        list.add(
                isWater ? FluidIngredient.of(fluidStack) : FluidIngredient.of(tag, fluidStack.getAmount()));
    }
    ...
    private static void copyToRecord(
            RecipeRecord record, RecipeRecord other, int count, int circuitMeta) {
        other
                .inputComponentItems()
                .forEach((component, integer) -> copyToRecord(other, component, integer, circuitMeta + 1));
        ...
        other
                .inputMaterialItems()
                .forEach(
                        (entry, integer) -> addToMap(record.inputMaterialItems(), entry, integer * count / 64));
        other
                .inputTagItems()
                .forEach((tag, integer) -> addToMap(record.inputTagItems(), tag, integer * count / 64));
        other
                .inputItems()
                .forEach(stack -> addStackToList(
                        record.inputItems(), stack.copyWithCount(stack.getCount() * count / 64)));
