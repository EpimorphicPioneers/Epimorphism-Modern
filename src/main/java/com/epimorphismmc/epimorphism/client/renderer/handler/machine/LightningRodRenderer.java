...
public class LightningRodRenderer extends MachineRenderer {
    ...
    public LightningRodRenderer(int tier, ResourceLocation workableModel) {
        ...
        this.setTextureOverride(Map.of(
                "bottom",
                GTCEu.id("block/casings/voltage/%s/bottom"
                        .formatted(GTValues.VN[tier].toLowerCase(Locale.ROOT))),
                "top",
                GTCEu.id("block/casings/fusion/fusion_casing"),
                "side",
                GTCEu.id("block/casings/voltage/%s/side"
                        .formatted(GTValues.VN[tier].toLowerCase(Locale.ROOT)))));
    ...
    public void renderMachine(
        ...
        if (!(side == Direction.UP || side == Direction.DOWN) && modelFacing != null) {
            quads.add(FaceQuad.bakeFace(
                    modelFacing,
                    ModelFactory.getBlockSprite(TransformerRenderer.ENERGY_OUT_HI),
                    modelState,
                    2));
        }
