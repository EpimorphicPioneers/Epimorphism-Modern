...
public class DigesterRenderer extends WorkableCasingMachineRenderer {
    ...
    private DigesterRenderer() {
        super(
                ...
                Epimorphism.id("block/multiblock/digester"),
                false);
    }
    ...
    public void renderMachine(
        ...
        if (machine instanceof IWorkable workable /*&& workable.isActive()*/ && side == null) {
            BLOCKS.forEach(aabb -> quads.add(FaceQuad.bakeFace(
                    aabb, Direction.UP, FluidHelper.getStillTexture(STACK), modelState, 2, 5, true, true)));
        }
    }
}
