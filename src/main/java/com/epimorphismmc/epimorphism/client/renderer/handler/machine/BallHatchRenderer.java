...
public class BallHatchRenderer extends TieredHullMachineRenderer {
    public static final ResourceLocation BALL_HATCH_OVERLAY =
            Epimorphism.id("block/overlay/machine/overlay_ball_hatch");
    public static final ResourceLocation IDLE = Epimorphism.id("block/multiblock/isa_mill/idle");
    public static final ResourceLocation SPINNING =
            Epimorphism.id("block/multiblock/isa_mill/spinning");
    ...
    public void renderMachine(
        ...
        if (side == frontFacing && modelFacing != null) {
            quads.add(FaceQuad.bakeFace(
                    modelFacing, ModelFactory.getBlockSprite(BALL_HATCH_OVERLAY), modelState));
                ...
                if (ballHatchMachine.isFormed()) {
                    if (ballHatchMachine.isWorking()) {
                        quads.add(FaceQuad.bakeFace(
                                AABB,
                                modelFacing,
                                ModelFactory.getBlockSprite(SPINNING),
                                modelState,
                                2,
                                0,
                                true,
                        ...
                        quads.add(FaceQuad.bakeFace(
                                AABB,
                                modelFacing,
                                ModelFactory.getBlockSprite(IDLE),
                                modelState,
                                2,
                                0,
                                true,
