    ...
    public void renderMachine(
        ...
        if (machine instanceof ProcessingArrayMachine processingArray && processingArray.isFormed()) {
            ...
            if (heldDefinition != null
                    && heldDefinition.getRenderer() instanceof MachineRenderer machineRenderer) {
                List<BakedQuad> machineQuad = new ArrayList<>();
                machineRenderer.renderMachine(
                        machineQuad, definition, machine, frontFacing, side, rand, modelFacing, modelState);
                for (var quad : machineQuad) {
                    var bakedQuad = Quad.from(quad);
                    for (int i = 0; i < 4; i++) {
                        var pos = bakedQuad.getVert(i);
                        bakedQuad = bakedQuad.withVert(
                                i,
                                new Vector3f(
                                        pos.x() - frontFacing.getStepX(),
                                        pos.y() + 1,
                                        pos.z() - frontFacing.getStepZ()));
                    }
