...
public class GeneralRecipeType extends GTRecipeType {
    ...
    private Iterator<GTRecipe> matchRecipe(
            GTRecipeType recipeType, int configuration, IRecipeCapabilityHolder holder) {
    ...
    public static GTRecipe addCircuitMeta(GTRecipe recipe, int configuration) {
        ...
        return CACHE.computeIfAbsent("%s_%s".formatted(recipe.id.toString(), configuration), key -> {
            ...
            newRecipe
                    .inputs
                    .computeIfAbsent(ItemRecipeCapability.CAP, capability -> new ArrayList<>())
                    .add(MORecipeHelper.itemContent(stack, 0, 0));
            return newRecipe;
        });
    }
}
