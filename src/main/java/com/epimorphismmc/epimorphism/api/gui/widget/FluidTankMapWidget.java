...
public class FluidTankMapWidget extends WidgetGroup
    ...
    public FluidTankMapWidget(
            int xPosition, int yPosition, int width, int height, TFFTMachine.TankStorage[] storages) {
        super(xPosition, yPosition, width, height);
        var group = (WidgetGroup) new WidgetGroup(xPosition, yPosition + 28, width, height - 28)
                .setBackground(GuiTextures.BACKGROUND_INVERSE);
        group.addWidget(
                fluidList = new DraggableScrollableWidgetGroup(
                                3, 3, group.getSize().width - 6, group.getSize().height - 6)
                        .setYScrollBarWidth(4)
                        .setYBarStyle(null, EPGuiTextures.BUTTON_NO_BORDER_LIGHT));
    ...
    private void addNewFluid(TFFTMachine.TankStorage storage) {
        ...
        IGuiTexture icon =
                ResourceTexture.fromSpirit(FluidHelper.getStillTexture(fluid).contents().name());
        int color = 0;

        if (!selectedMap.containsKey(uniqueID)) {
            var index = fluidList.widgets.size();
            var selectableWidgetGroup =
                    new SelectableWidgetGroup(0, index * 24, fluidList.getSize().width - 4, 24);
            ...
            selectableWidgetGroup.addWidget(new ToggleButtonWidget(
                    111, 3, 18, 18, EPGuiTextures.TOGGLE_BUTTON_LOCK, storage::isLocked, storage::setLocked));
            selectableWidgetGroup.addWidget(new ToggleButtonWidget(
                    131,
                    3,
                    18,
                    18,
                    EPGuiTextures.TOGGLE_BUTTON_VOID,
                    storage::isVoiding,
                    storage::setVoiding));
            selectableWidgetGroup.addWidget(new ToggleButtonWidget(
                    151,
                    ...
                    18,
                    18,
                    EPGuiTextures.TOGGLE_BUTTON_SELECTED,
                    storage::isSelected,
                    storage::setSelected));
