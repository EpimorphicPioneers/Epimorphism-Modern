...
public class FrackerMachine extends WorkableElectricMultiblockMachine {
    ...
    protected class FrackerLogic extends RecipeLogic {
        ...
        @Getter
        @Nullable private Fluid veinFluid;

        public FrackerLogic(FrackerMachine machine) {
            super(machine);
        }

        public static final FluidStack FRACKING_FLUID =
                EPMaterials.FracturingFluid.getFluid(FluidHelper.getBucket());
        ...
        @Nullable private GTRecipe getFrackingRecipe() {
            if (getLevel() instanceof ServerLevel serverLevel) {
                var data = BedrockFluidVeinSavedData.getOrCreate(serverLevel);
