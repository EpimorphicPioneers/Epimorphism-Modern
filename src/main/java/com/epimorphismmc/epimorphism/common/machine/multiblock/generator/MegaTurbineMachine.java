...
import com.epimorphismmc.epimorphism.api.machine.feature.multiblock.IReinforcedRotorHolder;
...
public class MegaTurbineMachine extends WorkableElectricMultiblockMachine
    ...
    @Nullable protected TickableSubscription rotorSpeedSubs;

    @Nullable private TickableSubscription setRotorSubs;
        ...
        for (IMultiPart part : getParts()) {
            ...
            for (var handler : part.getRecipeHandlers()) {
                ...
                if (handler.getCapability() == ItemRecipeCapability.CAP
                        && handler instanceof IItemTransfer itemTransfer) {
    ...
    protected @Nullable GTRecipe getRealRecipe(GTRecipe recipe) {
        ...
        if (checkRotor() && EUt > 0) {
            ...
            if (excessVoltage >= turbineMaxVoltage) {
                ...
                var parallelResult =
                        GTRecipeModifiers.fastParallel(this, recipe, Math.max(1, maxParallel), false);
                ...
                recipe.tickOutputs.put(
                        EURecipeCapability.CAP, List.of(new Content(eut, 1.0f, 0.0f, null, null)));
                return recipe;
            }
        }
        return null;
    ...
    public boolean onWorking() {
        ...
        if (getOffsetTimer() % 20 == 0) {
            var numMaintenanceProblems = getParts().stream()
                    ...
                    .orElse(0);
            rotorHolders.forEach(holder ->
                    holder.damageRotor((isHighSpeedMode() ? 12 : 1) * (1 + numMaintenanceProblems)));
        }
        ...
        for (IMultiPart part : getParts()) {
            if (isHighSpeedMode() && part instanceof IMaintenanceMachine maintenanceMachine) {
                if (ConfigHolder.INSTANCE.machines.enableMaintenance) {
                    maintenanceMachine.calculateMaintenance(
                            maintenanceMachine, 12 * getRecipeLogic().getProgress());
        ...
        if (isFormed()) {
            if (rotorHolders != null && !rotorHolders.isEmpty() && getRotorEfficiency() > 0) {
                textList.add(Component.translatable(
                        "gtceu.multiblock.turbine.rotor_speed",
                        FormattingUtil.formatNumbers(getRotorSpeed()),
                        FormattingUtil.formatNumbers(getMaxRotorHolderSpeed())));
                textList.add(
                        Component.translatable("gtceu.multiblock.turbine.efficiency", getTotalEfficiency()));
                ...
                if (isActive()) {
                    textList.add(
                            ...
                            Component.translatable(
                                    "gtceu.multiblock.turbine.energy_per_tick",
    ...
    public void attachConfigurators(ConfiguratorPanel configuratorPanel) {
        ...
        configuratorPanel.attachConfigurators((new IFancyConfiguratorButton.Toggle(
                        ...
                        this::isHighSpeedMode,
                        (clickData, pressed) -> this.setHighSpeedMode(pressed)))
                .setTooltipsSupplier(pressed -> List.of(Component.translatable(
                        pressed
                                ? "gui.epimorphism.high_speed.desc.enabled"
                                : "gui.epimorphism.high_speed.desc.disabled"))));
    }
        ...
        if (isInvUpdate() && !hasRotor()) {
            for (int index = 0; index < inputBuses.getSlots(); index++) {
                ...
                for (IReinforcedRotorHolder holder : rotorHolders) {
                    if (!holder.hasRotor()) {
                        var stack =
                                MOTransferUtils.extractItemAccountNotifiableList(inputBuses, index, 1, false);
    ...
    public int getTotalEfficiency() {
        return rotorHolders.stream()
                .mapToInt(IReinforcedRotorHolder::getTotalEfficiency)
                .min()
                .orElse(-1);
    }
    ...
    public int getRotorEfficiency() {
        return rotorHolders.stream()
                .mapToInt(IReinforcedRotorHolder::getRotorEfficiency)
                .min()
                .orElse(-1);
    }
    ...
    public int getRotorDurabilityPercent() {
        return rotorHolders.stream()
                .mapToInt(IReinforcedRotorHolder::getRotorDurabilityPercent)
                .min()
                .orElse(-1);
    }
    ...
    public int getTotalPower() {
        return rotorHolders.stream()
                .mapToInt(IReinforcedRotorHolder::getTotalPower)
                .min()
                .orElse(-1);
    }
