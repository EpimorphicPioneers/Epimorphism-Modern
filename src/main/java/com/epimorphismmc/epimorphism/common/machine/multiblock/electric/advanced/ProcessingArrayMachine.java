...
public class ProcessingArrayMachine extends TieredWorkableElectricMultiblockMachine
    ...
    // runtime
    @Nullable private GTRecipeType[] recipeTypeCache;
    ...
    protected NotifiableItemStackHandler createMachineStorage(Object... args) {
        var storage = new NotifiableItemStackHandler(
    ...
    protected boolean isMachineStack(ItemStack itemStack) {
        if (itemStack.getItem() instanceof MetaMachineItem metaMachineItem) {
            ...
            for (GTRecipeType type : recipeTypes) {
                if (GTRecipeTypes.ELECTRIC.equals(type.group)) {
                    return true;
                }
            }
        }
    ...
    @Nullable public MachineDefinition getMachineDefinition() {
        if (machineStorage.storage.getStackInSlot(0).getItem()
                instanceof MetaMachineItem metaMachineItem) {
    ...
    public int getOverclockTier() {
        ...
        int machineTier = machineDefinition == null
                ? getDefinition().getTier()
    ...
    @Nullable public static GTRecipe recipeModifier(MetaMachine machine, @Nonnull GTRecipe recipe, @NotNull OCParams params, @NotNull OCResult result) {
        if (machine instanceof ProcessingArrayMachine processingArray
            ...
            if (RecipeHelper.getRecipeEUtTier(recipe) > processingArray.getTier()) return null;

            int parallelLimit =
                    Math.min(processingArray.machineStorage.storage.getStackInSlot(0).getCount(), (int)
                            (processingArray.getMaxVoltage() / RecipeHelper.getInputEUt(recipe)));

            if (parallelLimit <= 0) return null;

            // apply parallel first
            var parallel = Objects.requireNonNull(GTRecipeModifiers.accurateParallel(
                    machine,
                    recipe,
                    Math.min(parallelLimit, getMachineLimit(machine.getDefinition().getTier())),
                    false));
            int parallelCount = parallel.getSecond();
            recipe = parallel.getFirst();

            // apply overclock afterward
            long maxVoltage = Math.min(
                    processingArray.getOverclockVoltage() * parallelCount, processingArray.getMaxVoltage());
            recipe =
                    RecipeHelper.applyOverclock(OverclockingLogic.NON_PERFECT_OVERCLOCK, recipe, maxVoltage, params, result);
    ...
    public Map<RecipeCapability<?>, Integer> getOutputLimits() {
        ...
        return GTRegistries.RECIPE_CAPABILITIES.values().stream()
                .map(key -> Map.entry(key, 0))
