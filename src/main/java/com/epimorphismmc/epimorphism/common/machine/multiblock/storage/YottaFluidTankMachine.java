...
import com.epimorphismmc.epimorphism.api.machine.ScheduledSubscriptionHandler;
...
public class YottaFluidTankMachine extends WorkableMultiblockMachine
        implements IFancyUIMachine, IDisplayUIMachine, ITankProvider {
    protected static final ManagedFieldHolder MANAGED_FIELD_HOLDER = new ManagedFieldHolder(
            YottaFluidTankMachine.class, WorkableMultiblockMachine.MANAGED_FIELD_HOLDER);
    ...
    public YottaFluidTankMachine(IMachineBlockEntity holder) {
        ...
        this.tickSubscription =
                new ScheduledSubscriptionHandler(this, this::transferFluid, this::isFormed);
    ...
    public void onStructureFormed() {
        ...
        IValueContainer<?> container = getMultiblockState()
                .getMatchContext()
                .getOrCreate("FluidTankCellValue", IValueContainer::noop);
        int glassTier = MOUtils.getOrDefault(
                () -> data instanceof ITierGlassType, () -> ((ITierGlassType) data).tier(), 0);
        int maxTier = MOUtils.getOrDefault(
                () -> container instanceof FluidTankCellContainer,
                () -> ((FluidTankCellContainer) container).getMaxTier(),
                0);
    ...
    public void attachConfigurators(ConfiguratorPanel configuratorPanel) {
        ...
        configuratorPanel.attachConfigurators((new IFancyConfiguratorButton.Toggle(
                        ...
                        this::isLocked,
                        (clickData, pressed) -> this.setLocked(pressed)))
        ...
        configuratorPanel.attachConfigurators((new IFancyConfiguratorButton.Toggle(
                        ...
                        this::isVoiding,
                        (clickData, pressed) -> this.setVoiding(pressed)))
                .setTooltipsSupplier((pressed) -> List.of(Component.translatable(
                        pressed
    ...
    public void addDisplayText(List<Component> textList) {
        ...
        if (isFormed()) {
            ...
            if (fluidTank != null) {
                ...
                textList.add(
                        Component.translatable("block.epimorphism.yotta_fluid_tank.fluid", getFluid()));
                textList.add(Component.translatable(
                                "block.epimorphism.yotta_fluid_tank.stored",
                                MOFormattingUtils.abbreviate2F(fluidStored))
                        .setStyle(Style.EMPTY.withHoverEvent(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT, Component.literal(getStored() + " L")))));
                textList.add(Component.translatable(
                                "block.epimorphism.yotta_fluid_tank.capacity",
                                MOFormattingUtils.abbreviate2F(fluidCapacity))
                        .setStyle(Style.EMPTY.withHoverEvent(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT, Component.literal(getCapacity() + " L")))));
            }
        }
    ...
    public void setLocked(boolean locked) {
        ...
        if (!isLocked) {
            ...
            getRecipeLogic()
                    .setStatus(fluidTank.hasFluid() ? RecipeLogic.Status.WORKING : RecipeLogic.Status.IDLE);
        }
    }
        ...
        if (!getLevel().isClientSide) {
            ...
            if (fluidTank.hasFluid()) {
                ...
                for (var handler : handlers) {
                    list = Objects.requireNonNullElseGet(
                            handler.handleRecipe(IO.IN, null, list, null, false), Collections::emptyList);
                }
                ...
                if (!EPFluidUtil.isDefaultFluid(inputFluidStack)) {
                    ...
                    if (fluidBanked != 0) {
                        ...
                        GTRecipeBuilder.ofRaw()
                                .inputFluids(inputFluidStack)
                                .buildRawRecipe()
                                .handleRecipeIO(IO.IN, this);
                    }
                }
            }
            ...
            if (!EPFluidUtil.isDefaultFluid(outputFluidStack) && outputFluidStack.getAmount() >= 0) {
                ...
                List<IRecipeHandler<?>> handlers = Objects.requireNonNullElseGet(
                        capabilitiesProxy.get(IO.OUT, GTRecipeCapabilities.FLUID), Collections::emptyList);
                for (var handler : handlers) {
                    list = Objects.requireNonNullElseGet(
                            handler.handleRecipe(IO.OUT, null, list, null, false), Collections::emptyList);
                }
                if (!list.isEmpty()) {
    ...
    public class YottaFluidTank extends MachineTrait implements ITankTrait {
        ...
        protected static final ManagedFieldHolder MANAGED_FIELD_HOLDER =
                new ManagedFieldHolder(YottaFluidTankMachine.YottaFluidTank.class);
        ...
        public BigInteger fill(
                Fluid fluid, BigInteger amount, boolean simulate, boolean notifyChanges) {
