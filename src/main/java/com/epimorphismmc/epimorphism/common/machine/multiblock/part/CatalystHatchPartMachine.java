...
public class CatalystHatchPartMachine extends TieredIOPartMachine {
    ...
    protected NotifiableItemStackHandler createInventory() {
        return new NotifiableItemStackHandler(
                this, 16, IO.IN, IO.OUT, slots -> new ItemStackTransfer(slots) {
                    @Override
            ...
            @Override
            @Nullable public List<Ingredient> handleRecipeInner(
                    IO io,
                    GTRecipe recipe,
    ...
    private void addSlots(
            WidgetGroup container, IItemTransfer handler, int x, int y, boolean canPutItems) {
        int index = 0;
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 4; i++) {
                container.addWidget(
                        new SlotWidget(handler, index++, x + i * 18, y + j * 18, true, canPutItems)
