...
public class FishingPondRecipeLogic extends RecipeLogic {
        ...
        if (getMachine().getLevel() instanceof ServerLevel) {
            ...
            if (match != null) {
                if (match.matchRecipe(this.machine).isSuccess()
                        && match.matchTickRecipe(this.machine).isSuccess()) {
                    setupRecipe(match);
                }
            }
        }
    ...
    @Nullable private GTRecipe getFishingRecipe() {
        ...
        var builder = GTRecipeBuilder.ofRaw().duration(MAX_PROGRESS).EUt(ENERGY);

        var tuple = GTRecipeModifiers.accurateParallel(
                machine,
                GTRecipeBuilder.ofRaw()
                        .copyFrom(builder)
                        .outputItems(Items.BARRIER.getDefaultInstance())
                        .buildRawRecipe(),
                (int) Math.min(machine.getParallelNumber(), machine.getMaxVoltage() / ENERGY),
                false);
        ...
        generatedLoot.stream().filter(itemStack -> !itemStack.isEmpty()).forEach(builder::outputItems);
        ...
        if (newRecipe != null) {
            if (newRecipe.matchRecipe(machine).isSuccess()
                    && newRecipe.matchTickRecipe(machine).isSuccess()) {
                return newRecipe;
            }
        }
        return recipe;
    ...
    private Boolean checkWater() {
        ...
        final int xDir = getMachine().getFrontFacing().getOpposite().getStepX() * mCurrentDirectionX;
        final int zDir = getMachine().getFrontFacing().getOpposite().getStepZ() * mCurrentDirectionZ;
