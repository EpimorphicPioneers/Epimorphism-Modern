...
public class VajraItem extends ComponentItem {
        ...
        if (ALWAYS_SUPPORTED_ACTIONS.contains(toolAction)) {
            ...
            if (electricItem != null) {
                long energyAvailable =
                        electricItem.discharge(DESTROY_ENERGY, GTValues.UV, true, false, true);
    ...
    //
    //        var electricItem = GTCapabilityHelper.getElectricItem(stack);
    //        FloatingLong energy = energyContainer == null ? FloatingLong.ZERO :
    ...
    //            //If we have enough energy to act at full damage, use the cached multimap rather
    ...
    // "Weapon modifier", MekanismConfig.gear.disassemblerAttackSpeed.get(), Operation.ADDITION));
    ...
    public static boolean canDestroyBlock(ItemStack stack) {
        var electricItem = GTCapabilityHelper.getElectricItem(stack);
        if (electricItem != null) {
            return electricItem.discharge(DESTROY_ENERGY, GTValues.UV, true, false, true)
                    >= DESTROY_ENERGY;
        }
        return false;
    }
