...
public class EPModels {
    ...
    public static <T extends Item>
            NonNullBiConsumer<DataGenContext<Item, T>, RegistrateItemModelProvider> wrapItemModel(
                    ResourceLocation wrapped) {
        return (ctx, prov) -> prov.getBuilder("item/" + prov.name(ctx::getEntry))
                .parent(new ModelFile.UncheckedModelFile(Epimorphism.id("item/wrap_item")))
    ...
    public static void captureToolModel(
            DataGenContext<Item, ? extends Item> ctx, RegistrateItemModelProvider prov) {
        // empty model
        ...
        prov.generated(ctx::getEntry, prov.modLoc("item/%s/empty".formatted(prov.name(ctx))))
                .override()
                .predicate(Epimorphism.id("organism_capture_tool"), 0)
                .model(new ModelFile.UncheckedModelFile(
                        prov.modLoc("item/%s_empty".formatted(prov.name(ctx)))))
                .end()
                .override()
                .predicate(Epimorphism.id("organism_capture_tool"), 1)
                .model(new ModelFile.UncheckedModelFile(
                        prov.modLoc("item/%s_filled".formatted(prov.name(ctx)))))
                .end();
    }
}
